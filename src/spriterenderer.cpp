...
namespace LWRL
{
	...
	Quad SpriteRenderer::GenerateQuad(glm::vec3 pos, glm::vec4 color, Texture* texture)
	{
	...
	void SpriteRenderer::RenderQuad(Quad quad, Texture* texture)
	{
		texture->used = true;
		...
		batch.buffer[batch.index] = quad.left;
		batch.index++;

		batch.buffer[batch.index] = quad.right;
		batch.index++;
